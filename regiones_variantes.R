...
datos <- read_excel(file_in)


start <- ymd("2021-12-01")
end <- as.Date("2022-07-16")

datos$Fecha_de_recoleccion <- lubridate::ymd(datos$Fecha_de_recoleccion)

datos <- datos[datos$Fecha_de_recoleccion >= start & datos$Fecha_de_recoleccion <= end,]

datos<- datos[datos$Fecha_de_recoleccion >= start & datos$Fecha_de_recoleccion <= end,]

Mes_recoleccion <- format_ISO8601(datos$Fecha_de_recoleccion, precision = "ym") 
Mes_recoleccion_decimal <- floor_date(datos$Fecha_de_recoleccion, unit = "month") %>% decimal_date()
Semana_epi <- epiweek(datos$Fecha_de_recoleccion)
datos<- cbind(datos, Mes_recoleccion = c(Mes_recoleccion),
...
datos <- datos %>%
  mutate(linajeOMS=case_when(Linaje== "B.1.1.7" ~ "Alpha", Linaje=="B.1.1.519" ~ "B.1.1.519", Linaje=="B.1.1.222" ~ "B.1.1.222",
                               ...
                               Linaje=="BA.2.2"~"BA.2.2",Linaje=="BA.2.21"~"BA.2.21",Linaje=="BA.2.22"~"BA.2.22",Linaje=="BA.2.23"~"BA.2.23",Linaje=="BA.2.26"~"BA.2.26",Linaje=="BA.2.3.2"~"BA.2.3.2", Linaje=="BA.2.3.3"~"BA.2.3.3", Linaje=="BA.2.3.4"~"BA.2.3.4", Linaje=="BA.2.2.31"~"BA.2.2.31",Linaje=="BA.4"~"BA.4", Linaje=="BA.5"~"BA.5",  Linaje=="BA.5.3.1"~"BA.5.3.1",Linaje=="BA.5.5"~"BA.5.5",Linaje=="BB.2"~"BB.2",Linaje=="BF.1"~"BF.1",Linaje=="BG.1"~"BG.1",Linaje=="BG.2"~"BG.2", Linaje=="BA.5.2.1"~"BA.5.2.1", Linaje=="BA.2.3.3"~"BA.2.3.3", Linaje=="BA.2.4"~"BA.2.4", Linaje=="BA.2.22"~"BA.2.22", Linaje=="BA.2.9"~"BA.2.9",
                               TRUE  ~ "Other"))
...
filter1 <-  datos %>% group_by(Estado,Fecha_de_recoleccion, Mes_recoleccion, linajeOMS) %>% summarise(n = n()) %>% mutate(percentage = n/sum(n)*100)
filter2<-filter(filter1, percentage>1)

#color=c("BA.1"="lightslateblue", "BA.1.1"="skyblue", "BA.1.1.8"="peru", "BA.1.14"="azure4", "BA.1.15"="magenta3", "BA.1.17"="purple","BA.1.17.2"="orchid4","BA.1.20"="sienna3", "BA.1.21"= "red2", "BA.2"="yellowgreen", "BA.2.1"="antiquewhite", "BA.2.12"="indianred4","BA.2.26"="thistle3", "BA.5.2.1"="springgren4","BA.2.13"="honeydew", "BA.2.3"="darkseagreen3","BA.2.3"="red", "BA.2.3.2"="#8B3626","BA.2.9"="pink", "BA.4"="gainsboro", "BA.5"="lightgreen","Delta"="darkgoldenrod", "BA.5.3.1"="orange2","BA.5.5"="pink2","BB.2"="#63B8FF","BF.1"="#FFA54F","BG.1"="#8B8B00","BG.2"="#8B2252", "Other"="slateblue4")
